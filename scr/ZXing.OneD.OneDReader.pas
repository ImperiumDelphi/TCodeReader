...
tryHarderWithoutRotation := (hints <> nil) and (hints.ContainsKey(ZXing.DecodeHintType.TRY_HARDER_WITHOUT_ROTATION));
...
if (tryHarder and ((not tryHarderWithoutRotation) and image.RotateSupported)) then
   ...
   orientation := 270;
   if ((metadata <> nil) and metadata.ContainsKey(ZXing.ResultMetadataType.orientation)) then
      begin
      // But if we found it reversed in doDecode(), add in that result here:
      orientation := (orientation + (metadata[ZXing.ResultMetadataType.orientation] as IIntegerMetadata).Value) mod 360;
      end;
   ...
   for attempt := 0 to 1 do
      ...
      if (attempt = 1) then
         ...
         if needsCallBack then
            begin
            hints.TryGetValue(ZXing.DecodeHintType.NEED_RESULT_POINT_CALLBACK, obj);
