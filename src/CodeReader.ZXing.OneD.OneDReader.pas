...
unit CodeReader.ZXing.OneD.OneDReader;
...
type
  ...
  public
    ...
    /// String which the barcode encodes
    /// </returns>
    function decode(const image: TBinaryBitmap; hints: TDictionary<TDecodeHintType, TObject>): TReadResult; overload; virtual;
...
function TOneDReader.decode(const image: TBinaryBitmap; hints: TDictionary<TDecodeHintType, TObject>): TReadResult;
var
...
tryHarderWithoutRotation := (hints <> nil) and (hints.ContainsKey(CodeReader.ZXing.DecodeHintType.TRY_HARDER_WITHOUT_ROTATION));

if (tryHarder and ((not tryHarderWithoutRotation) and image.RotateSupported)) then
   begin
   ...
   orientation := 270;
   if ((metadata <> nil) and metadata.ContainsKey(CodeReader.ZXing.ResultMetadataType.orientation)) then
      begin
      // But if we found it reversed in doDecode(), add in that result here:
      orientation := (orientation + (metadata[CodeReader.ZXing.ResultMetadataType.orientation] as IIntegerMetadata).Value) mod 360;
      end;

   Result.putMetadata(CodeReader.ZXing.ResultMetadataType.orientation, TResultMetadata.CreateIntegerMetadata(orientation));
...
function TOneDReader.doDecode(const image: TBinaryBitmap; hints: TDictionary<TDecodeHintType, TObject>): TReadResult;
...
begin
needsCallBack := (hints <> nil) and (hints.ContainsKey(CodeReader.ZXing.DecodeHintType.NEED_RESULT_POINT_CALLBACK));
width := image.width;
height := image.height;
   ...
   for attempt := 0 to 1 do
      ...
      if (attempt = 1) then
         ...
         if needsCallBack then
            begin
            hints.TryGetValue(CodeReader.ZXing.DecodeHintType.NEED_RESULT_POINT_CALLBACK, obj);
            hints.Remove(CodeReader.ZXing.DecodeHintType.NEED_RESULT_POINT_CALLBACK);
      ...
      if hadResultPointCallBack then
         begin
         hints.Add(CodeReader.ZXing.DecodeHintType.NEED_RESULT_POINT_CALLBACK, obj);
         end;
