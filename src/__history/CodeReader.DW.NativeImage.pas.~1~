unit CodeReader.DW.NativeImage;
...
uses
  ...
  CodeReader.DW.NativeImage.Win;
  {$ENDIF}
  {$IF Defined(IOS)}
  CodeReader.DW.NativeImage.iOS;
  {$ENDIF}
  {$IF Defined(ANDROID)}
  CodeReader.DW.NativeImage.Android;
  {$ENDIF}
...
procedure TCustomNativeImage.LoadFromBitmap(const ABitmap : TBitmap);
Var
   Str : TMemoryStream;
begin
Str := TMemoryStream.Create;
ABitmap.SaveToStream(Str);
Str.Position := 0;
Model.LoadFromStream(Str);
Str.DisposeOf;
end;
